...
## Задача 3
 - Направете копие на класа Pair от Задача 2 и го наименовайте NumericPair.

 - Променете темплейта, така че first и second да са от един и същ произволен тип (т.е. template < typename T > ... T first; T second;).

 - Създайте concept с име Number, който позволява само цели числа и числа с плаваща запетая (използвайте std::integral и std::floating_point).

 - Направете съответните промени в класа NumericPair така че типът T да удовлетворява concept-а Number.

 - Добавете валидации в setter-ите на first и second, които при отрицателен аргумент хвърлят грешка std::invalid_argument.

 - Създайте "custom" exception, наследник на std::runtime_error, който да се казва DivisionByZeroException. Направете default constructor със съобщение "Division by zero" и контруктор с параметри, приемащ const string& message.

 - Добавете нова член-функция в класа NumericPair: T divide() {...}, която връща резултата от first/second, ако second е 0 да се хвърли "custom" exception-а DivisionByZeroException.

 - Копирайте следния код и направете нужните корекции, за да обработва правилно хвърлянето на грешки.
